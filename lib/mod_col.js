...
//
// modified by Ricardo Alcocer - alco@ricardoalcocer.com - to work with Titanium 
...
// receives the raw json data and returns a clean object with only the data needed by the caller
COL.prototype.getItemListDataArray=function(rawJsonData,startAt){
	...
	for(var i=0;i<totalRecs;i++){
		...
		// sometimes the "pueblo" entry is not present
		try{
			var itemCity=theData.query.results.table.tr[i].td[0].a.span[0].content;
			itemCity=itemCity.replace('Pueblo: ','');	
		}catch(e){
			itemCity='';
		}
		
		// sometimes the "precio" entry is not present
		try{
			var itemPrice=theData.query.results.table.tr[i].td[0].a.span[1].content;
			itemPrice=itemPrice.replace('Precio: ','');	
		}catch(e){
			itemPrice='';
		}

		// the source JSON data doesn't includes the item id as a property
		// we need to extract it from the link URL
...
// receives the raw json data and returns a clean object with only the data needed by the caller
COL.prototype.getItemData=function(rawJsonData){
	var theData=JSON.parse(rawJsonData.data);
	var itemName=theData.query.results.table.tr[0].td[1].span.content;
	var itemDescription=theData.query.results.table.tr[1].td.p.content;
	var itemImage="http://clasificadosonline.com" + theData.query.results.table.tr[0].td[0].a.img.src;
	var itemContact=theData.query.results.table.tr[0].td[1].p.span[0].content;
	var itemContactPhone=theData.query.results.table.tr[0].td[1].p.span[1].content;
	var itemPrice=theData.query.results.table.tr[0].td[1].p.content;
	itemPrice=itemPrice.replace('Precio: ','');
	itemPrice=itemPrice.replace('Contacto','');
...
// returns an array of category value pairs
COL.prototype.getCategories=function(){
	// categories are hardcoded in this array
	return [
               {
                  "value":"0",
                  "content":"Todos/All"
